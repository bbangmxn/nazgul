...
이 수치는 `초기 기술 검증 단계에서 확보한 관측값`입니다. 아직 데이터셋과 평가 기준이 충분히 정리되지 않았기 때문에, 이 프로젝트는 완성된 성능 보고서가 아니라 `RAG 구조를 실험하는 연구 프로토타입`으로 보는 것이 맞습니다.
...
Nazgul은 처음에는 취미 기반 SNS를 목표로 개발한 프로젝트였습니다.  
하지만 기존 RAG가 가지는 한계와 비판을 확인하면서, 단순 검색을 넘어서 도메인과 사용자 맥락을 반영하는 개인화된 RAG 구조를 직접 설계하고 검증해보고자 했습니다.  
현재는 기존 SNS 도메인을 기반으로, 취미 커뮤니티에서 동작하는 RAG 검색·추천 프로토타입을 개발하며 관련 구조를 연구하고 있습니다.

추후 온톨로지와 RAG의 결합을 한번 테스트 해보고 싶어 하나의 발판으로써 더 올릴 예정입
