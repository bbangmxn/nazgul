...
**원래 서버는 문제가 부족한 점이 많다고 생각을 해서 RAG 기반 sns로 테스트 버전으로 추후 업데이트 할 예정입니다**
...
RAG를 붙인 이유는 실제로 다양한 RAG를 보았을때 실제 그래프 타입의 RAG 부터 일반 선형 구조의 RAG 같은 부분이 많았고, 
RAG에 대한 회의론이 많이 일어나기 시작을 했기에 컴퓨터를 RAG 형태로 해보기 위해
이론을 조사하다 보니 sns에 적용해서 RAG에 테스트를 해보는게 어떤가 라는 생각을 해봤습니다.
추후 온톨로지의 개념을 더 공부하고 RAG와의 결합까지 하는 방향성이 궁금해 다른 프로젝트에서 온톨로지와 RAG의 결합을 위한 개인화 RAG를 개발 연구 진행 예정입니다
